...
### Политика обработки медиа (решено 2026-07-19)

| Тип | Обработка | Хранится |
|---|---|---|
| Фото | Декод с `imageOrientation:'from-image'` → canvas → **WebP q0.82** (JPEG-фолбэк), длинная сторона ≤2560px, **без апскейла** | одна версия + превью 640px q0.75 |
| HEIC | Конвертация в браузере через **libheif WASM** (`heic2any` с jsdelivr), далее как фото. Требует `'wasm-unsafe-eval'` в CSP страниц загрузки | как фото |
| GIF | Оригинал (анимация сохраняется), превью — первый кадр | оригинал + превью |
| Видео | Пока как есть + постер. **Следующий шаг — перекодирование в MP4/H.264 через WebCodecs** (проверено: `VideoEncoder.isConfigSupported('avc1.42001f')` = true; ffmpeg.wasm отклонён — 25 МБ и нужны COOP/COEP, которых GitHub Pages не даёт; внешний сервис — если WebCodecs не осилит) | оригинал + постер |
| Аудио | Как есть, читается длительность | оригинал |

Решения и следствия:
- **Оригиналы фото не хранятся** — только версия ≤2560px. HD-загрузка с сохранением исходника отложена до платного тарифа (на free 1 ГБ хранилища ≈ 2.5–3 тыс. фото).
- **EXIF стирается при перекодировании, включая GPS** — геометки из снимков не утекают. Это побочный, но желательный эффект.
- `imageOrientation` задаётся явно: текущий Chromium и так применяет поворот по умолчанию (проверено на JPEG с меткой Orientation=6), но в старых сборках дефолт был `none` — явное значение делает поведение одинаковым везде.
- Пока файл конвертируется/грузится, редактор показывает стадию («Converting from HEIC…» / «Processing…» / «Uploading…»), а **кнопка публикации заблокирована** — альбом остаётся черновиком до готовности.

Лимиты: исходное фото ≤ 30 МБ (после сжатия всегда сотни КБ, ограничение только про память браузера), аудио ≤ 20 МБ, видео ≤ 50 МБ (потолок файла на free-тарифе). Постер видео и превью фото генерирует клиент.
