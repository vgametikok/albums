...
| Хостинг | GitHub Pages, репо `Poklontsevv/albums` → `https://poklontsevv.github.io/albums/` |
...
- **Frontend**: push в `Poklontsevv/albums` (ветка main, корень) → Pages. Внимание: это **другой GitHub-аккаунт**, чем sharky (vgametikok) — пуш под учёткой Poklontsevv. Локальный репо `D:\Albums` уже `git init`, remote `origin` настроен, `credential.username=Poklontsevv` прописан локально (GCM закэширует эту сессию отдельно от vgametikok при первом реальном push).
...
- **Фаза 0 — ручное (юзер)**: репо + Pages; в Supabase: включить Google-провайдер (Client ID/Secret из Google Cloud Console, redirect `https://<ref>.supabase.co/auth/v1/callback`), URL Configuration (§8.6); дать доступ к БД (§10) + сообщить project ref; убрать sb_secret из любых заметок, позже ротировать.
